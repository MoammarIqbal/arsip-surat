...
<x-layouts.app :title="$title">
  ...
  <script>
    ...
    // ============ Tambahkan kolom Aksi (hanya admin) ============
    ...
    function render(list) {
      ...
      rows.innerHTML = list.map((r, i) => {
        ...
        // Dokumen: jika link → Lihat, jika file → tombol Unduh
        const doc = r.tautan_dokumen
          ? `<a href="${r.tautan_dokumen}" target="_blank" class="text-blue-600 underline">Lihat</a>`
          : (r.file_path
              ? `<button
                   type="button"
                   class="px-2 py-1 border rounded hover:bg-gray-50 text-blue-700 btn-download"
                   data-id="${r.id}"
                   data-no="${r.nomor_surat}"
                   data-ext="${(r.file_path.split('.').pop() || '').toLowerCase()}">
                   Unduh
                 </button>`
      ...
      if (isAdmin) {
        document.querySelectorAll('.btn-del').forEach(btn => {
          btn.addEventListener('click', async () => {
            ...
            try {
              await apiFetch('/api/surat/' + id, { method: 'DELETE' });
              await loadData();
            } catch (e) {
      ...
      document.querySelectorAll('.btn-download').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            await downloadLetterFile(id, no, ext);
          } catch (e) {
    ...
    async function downloadLetterFile(id, nomorSurat, extHint) {
      ...
      // Ambil nama file dari header atau fallback nomor surat
      let filename = '';
      ...
      if (m) {
        ...
        const safe = safeName(nomorSurat);
        const ext  = (extHint && extHint.length <= 5) ? `.${extHint}` : '';
        filename = `${safe}${ext}`;
      }
    ...
    function updateSortIcons() {
      thElements.forEach(th => {
        ...
        if (field === sortField) {
          const arrow = sortAsc ? '▲' : '▼';
          th.innerHTML += ` <span class="text-gray-400">${arrow}</span>`;
        }
      });
    }
    ...
    async function loadData(qStr = '') {
      try {
        const url = qStr
          ? `/api/surat?per_page=100&search=${encodeURIComponent(qStr)}`
          : `/api/surat?per_page=100`;
